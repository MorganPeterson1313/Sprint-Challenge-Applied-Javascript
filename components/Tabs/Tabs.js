class TabLink {
  constructor(tabElement){
    ...
    // Get the `data-tab` value from this.tabElement and store it here
     ...
     //this.tabData = this.tabElement.querySelectorAll(`.card[data-tab= "${this.tabData}"]`) ;
    ...
    if(`'${this.tabData}'` === 'all'){
     ...
     this.cards = document.querySelectorAll('.cards-container .card') ;
      ...
      this.cards = document.querySelectorAll(`.card[data-tab= "${this.tabData}"]`=== this.tabData);
  ...
  selectTab(){
    ...
    // Iterate through the NodeList setting the display style each one to 'none'
     cards.forEach(card => card.style.dispaly = "none");
